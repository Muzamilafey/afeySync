...
import type { ServiceItem } from '@/features/billing/types';
...
export function CheckInForm({ preset, onDone }: { preset?: Patient | null; onDone?: () => void }) {
  ...
  const [f, setF] = useState({ type: 'opd', priority: 'normal', payer: 'cash', scheme: '', memberNumber: '', complaint: '', firstStage: 'triage', referralFrom: '' });
      ...
      (
        await api<{ visit: { _id: string; visitNumber: string }; queueEntry: { ticket: string } }>('/visits', {
          ...
          body: {
            ...
            type: f.type,
            priority: f.priority,
            payer: { type: f.payer, scheme: f.scheme || undefined, memberNumber: f.memberNumber || undefined },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Field label="Payer"><Select value={f.payer} onChange={(e) => setF({ ...f, payer: e.target.value })}><option value="cash">Cash</option><option value="sha">SHA</option><option value="insurance">Insurance</option><option value="corporate">Corporate</option></Select></Field>
        {f.payer !== 'cash' && f.payer !== 'sha' && <Field label="Scheme"><Input value={f.scheme} onChange={(e) => setF({ ...f, scheme: e.target.value })} /></Field>}
      ...
      <Field label="Charges at check-in (e.g. registration / consultation fee)"><ServicePicker priceList={f.payer === 'sha' ? 'sha' : f.payer === 'cash' ? 'cash' : 'insurance'} onPick={(s) => setCharges([...charges.filter((c) => c.code !== s.code), s])} /></Field>
      ...
      <Button onClick={() => m.mutate()} disabled={!patient} loading={m.isPending}>Check in</Button>
    </div>
  );
}
