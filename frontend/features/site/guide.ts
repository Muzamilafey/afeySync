    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'M-Pesa: paybill or till', text: 'Under “Customers pay into”, choose Paybill or Till number. For a paybill, enter the paybill number. For a till, enter the store (head office) number from your Daraja Go-Live email and the till number customers pay into. The passkey must belong to the paybill or store number.' },
          { title: 'M-Pesa is your own', text: 'M-Pesa (Daraja) and Pay Hero are set up by your facility for its own account: nobody at AfeySync needs to switch them on, and the money goes straight to your paybill, till or bank. Fill in the details and click “Save & switch on”.' },
          { title: 'Pay Hero', text: 'Pay Hero is a second way to send M-Pesa prompts. In Pay Hero, open Developers → API Keys → Generate key and copy the API key username and password (both required; the API secret is optional). Also note your payment channel ID (Payment Channels → My Payment Channels). Enter them, then choose whether Pay Hero sends every prompt or only stands in when M-Pesa (Daraja) is not set up. “Test connection” shows the channel the money goes to.' },
          { title: 'API base URL', text: 'It fills in by itself when you choose the environment (sandbox or production). Leave it unless Safaricom gives you a different address; a sandbox address is refused on a production setup.' },
          { title: 'Test', text: 'Click “Test connection”. For M-Pesa it also tells you where prompts will pay into, e.g. “Till 1731901”. Credentials are stored encrypted and are never shown in full again.' },
        ],
