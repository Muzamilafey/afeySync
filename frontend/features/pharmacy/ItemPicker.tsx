    ...
    <div className="relative">
      ...
      {q.length >= 2 && r.data && (
        <ul className="surface absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-md shadow-lg">
          ...
          {r.data.map((i) => (
            <li key={i._id}>
              <button type="button" className="flex w-full justify-between px-3 py-2 text-left text-sm hover:bg-[var(--surface-2)]" onClick={() => { onPick(i); setQ(''); }}>
                <span>{i.name} {i.strength} <span className="muted text-xs">{i.form} · {i.code}</span></span>
                <span className={`text-xs ${(i.stock?.usable ?? 0) > 0 ? 'text-emerald-600' : 'text-red-600'}`}>{i.stock?.usable ?? 0} {i.unit}</span>
              </button>
            </li>
          ))}
        </ul>
