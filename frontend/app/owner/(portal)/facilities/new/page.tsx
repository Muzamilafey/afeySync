...
export default function NewFacilityWizard() {
  ...
  const [integrations, setIntegrations] = useState({ sha: true, dha: true, mpesa: false, africastalking: false, smtp: false });
          ...
          <div className="space-y-3">
            {(Object.keys(integrations) as Array<keyof typeof integrations>).map((k) => (
              <label key={k} className="flex items-center gap-3 text-sm">
                ...
                <span className="font-medium">{{ sha: 'SHA', dha: 'DHA HIE', mpesa: 'M-Pesa', africastalking: "Africa's Talking SMS", smtp: 'SMTP email' }[k]}</span>
              </label>
            ))}
