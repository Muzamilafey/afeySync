    ...
    <>
      ...
      <Card>
        <div className="mb-3 grid gap-2 sm:grid-cols-3 lg:grid-cols-7">
          ...
          <Select value={f.provider} onChange={(e) => set('provider', e.target.value)}><option value="">All providers</option>{['sha', 'dha', 'mpesa', 'africastalking', 'smtp'].map((p) => <option key={p}>{p}</option>)}</Select>
